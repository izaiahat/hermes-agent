...
def _render_skills_index(
    ...
    return (
        "## Skills (mandatory)\n"
        "Before replying, scan the skills below. If a skill strongly matches the task domain, tool, or "
        "requested workflow, you MUST load it with skill_view(name) and follow its instructions. "
        ...
        "code review, planning, and testing — load them when the requested phase actually matches.\n"
        "Whenever the user asks you to configure, set up, install, enable, disable, modify, or troubleshoot "
        "Hermes Agent itself — its CLI, config, models, providers, tools, skills, voice, gateway, plugins, "
        "or any feature — load the `hermes-agent` skill first. It has the actual commands "
        "(e.g. `hermes config set …`, `hermes tools`, `hermes setup`) so you don't have to guess or invent "
        "workarounds.\n"
