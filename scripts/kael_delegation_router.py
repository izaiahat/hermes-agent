...
def make_codex(command: str, reasons: list[str]) -> RoutingDecision:
    return RoutingDecision(
        lane=CODEX_CLI["lane"],
        reasons=reasons,
        role=CODEX_CLI["role"],
...
def choose_parallel_child(profile: TaskProfile) -> tuple[str, str | None, list[str] | None, str | None]:
    ...
    if profile.file_count >= 5 or subtask_tokens > 300_000 or profile.clean_room:
        return CODEX_CLI["lane"], None, None, CODEX_CLI["command_read_only"]
    return (
